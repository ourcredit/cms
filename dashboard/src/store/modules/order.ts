...
class PointModule extends ListModule<IDeviceState, any, any> {
  ...
  actions = {
    ...
    async getopeAll(
      ...
      context.state.loading = true;
      let reponse: any = await Ajax.post("/api/order/opeorders", payload.data);
      context.state.loading = false;
      let page: PageResult<any> = reponse.data as PageResult<any>;
      context.state.totalCount = page.total;
      context.state.opeOrders = page.records;
    },
    async modify(
      context: ActionContext<IDeviceState, any>,
      payload: any
    ...
    async superback(
      ...
      await Ajax.get(`/api/order/superback/${payload.data}`);
    }
