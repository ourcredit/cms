...
class OperaterModule extends ListModule<IOperaterState, any, Operater> {
  ...
  actions = {
    ...
    async getAllAreas(
      ...
      context.state.loading = true;
      let reponse: any = await Ajax.post("/api/operater/allregions", {});
      context.state.loading = false;
      let page: any = reponse.data as any;
      context.state.areas = page;
    },
    async getAreas(
      context: ActionContext<IOperaterState, any>
    ): Promise<any> {
      context.state.loading = true;
      let reponse: any = await Ajax.post("/api/post/area", {});
      context.state.loading = false;
      let page: any = reponse.data as any;
      context.state.allareas = page;
    },
    async all(
      context: ActionContext<IOperaterState, any>,
      payload: any
    ...
    async get(
      ...
      let reponse: any = await Ajax.get("/api/operater/" + payload.data);
