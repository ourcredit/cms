...
public class CustomerOrderServiceImpl extends ServiceImpl<CustomerOrderRepository, CustomerOrder> implements ICustomerOrderService {
    ...
    public SortedMap<String, Object> wxPay(CustomerOrder input) {
        try {
            ...
            //PayUtil.httpRequest(WxPayConfig.pay_url, "POST", xml);
                    logger.error("调试模式_统一下单接口 返回XML数据：" + result);
    ...
    @Override
    public Page<CustomerOrder> selectOrders(Page<CustomerOrder> page, String operaterId) {
        Page<CustomerOrder> p = new Page<>();
        List<CustomerOrder> r = _orderRepository.selectCustomerOrderList(page, operaterId);
        p.setRecords(r);
